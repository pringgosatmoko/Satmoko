...
export const SystemLogs: React.FC<SystemLogsProps> = ({ onBack }) => {
  ...
  const runAudit = async () => {
    addLog("Memulai cek kondisi sistem...");
    await new Promise(r => setTimeout(r, 500));
    
    addLog("Versi Software: 6.5.0 terdeteksi");
    addLog("Koneksi: Menghubungkan ke pusat data...");
    
    try {
      const { data, error } = await supabase.from('members').select('id').limit(1);
      if (error) throw error;
      addLog("Database: Berjalan dengan baik");
    } catch {
      addLog("Database: Gangguan koneksi!");
    }

    addLog("AI Brain: Mengecek kunci akses...");
    const hasEnvKey = !!process.env.API_KEY;
    if (hasEnvKey) {
      addLog("AI Brain: Kunci akses aktif (Mode Premium)");
    } else {
      addLog("AI Brain: Kunci tidak ditemukan (Mode Standar)");
    }

    addLog("Keamanan: Protokol admin aktif");
    addLog("Laporan: Semua sistem siap digunakan.");
  };
    ...
    <div className="space-y-6 flex flex-col h-[calc(100vh-140px)]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-2xl font-black italic uppercase tracking-tighter">Cek <span className="text-cyan-400">Sistem</span></h2>
        </div>
