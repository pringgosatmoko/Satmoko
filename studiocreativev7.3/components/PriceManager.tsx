...
export const PriceManager: React.FC<PriceManagerProps> = ({ onBack, lang }) => {
  ...
  const priceItems = [
    { key: 'cost_image', icon: 'fa-image', title: t.image, variant: 'cyan' },
    { key: 'cost_video', icon: 'fa-video', title: t.video, variant: 'purple' },
    { key: 'cost_voice', icon: 'fa-microphone-lines', title: t.voice, variant: 'cyan' },
    { key: 'cost_studio', icon: 'fa-film', title: t.studio, variant: 'yellow' },
  ];
    ...
    <div className="flex flex-col h-[calc(100vh-140px)] gap-6 max-w-2xl mx-auto overflow-hidden">
      ...
      <div className="flex-1 glass-panel rounded-[2.5rem] bg-[#0d1117]/60 border-white/5 overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-2">
           {priceItems.map((item) => (
             <PriceTableRow key={item.key} {...item} value={prices[item.key]} onSync={(v) => handleUpdate(item.key, v)} lang={lang} isLoading={isLoading} />
           ))}
        </div>
      </div>
    </div>
...
const PriceTableRow: React.FC<RowProps> = ({ icon, title, value, onSync, variant, lang, isLoading }) => {
