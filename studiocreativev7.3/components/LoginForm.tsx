...
export const LoginForm: React.FC<LoginFormProps> = ({ onSuccess, lang }) => {
  ...
  const t = {
    id: {
      loginTitle: "MASUK KE STUDIO",
      regTitle: "DAFTAR AKUN BARU",
      name: "NAMA LENGKAP",
      email: "EMAIL",
      pass: "SANDI",
      plan: "PAKET",
      submitLogin: "MASUK STUDIO",
      submitReg: "DAFTAR SEKARANG",
      noAccount: "BELUM PUNYA AKUN?",
      haveAccount: "KEMBALI KE LOGIN",
      regSuccess: "DAFTAR BERHASIL: Tunggu persetujuan admin ya.",
    ...
    try {
      if (isRegister) {
        ...
        if (memberError || !memberData || memberData.status !== 'active') {
          ...
          throw new Error(lang === 'id' ? "AKSES DITOLAK: Hubungi Admin untuk aktivasi." : "ACCESS DENIED: Contact Admin for activation.");
        }
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="w-full glass-panel p-8 rounded-[2rem] bg-black/60 border border-white/5 shadow-2xl relative overflow-hidden">
        <div className="mb-6">
          <h2 className="text-[10px] font-black uppercase text-slate-500 tracking-[0.4em] text-center">
        ...
        <AnimatePresence mode="wait">
          {error && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4 p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-[9px] text-center font-black uppercase">
              {error}
            </motion.div>
          )}
          ...
          {successMsg ? (
            <div className="space-y-6 text-center py-2">
              ...
              <div className="space-y-3">
                <a href="https://wa.me/6285147007574" target="_blank" className="block w-full py-4 rounded-xl bg-cyan-500 text-black font-black uppercase text-[10px] tracking-widest shadow-lg shadow-cyan-500/20">{t.contactAdmin}</a>
                <button onClick={() => setSuccessMsg('')} className="text-[9px] font-black uppercase text-slate-600 hover:text-white transition-colors">{t.back}</button>
              </div>
            </div>
          ) : (
      ...
      {!successMsg && (
        <button onClick={() => setIsRegister(!isRegister)} className="mt-8 text-[9px] font-black uppercase tracking-[0.2em] text-slate-600 hover:text-cyan-400 transition-colors">
