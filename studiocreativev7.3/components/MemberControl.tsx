...
import { supabase, getActiveApiKey, getSlotStatus, isUserOnline, topupCredits } from '../lib/api';
...
export const MemberControl: React.FC<MemberControlProps> = ({ onBack, lang }) => {
  ...
  const t = {
    id: {
      title: "Kelola Member",
      subtitle: "Daftar Pengguna Studio",
      nodes: "PENGGUNA",
      health: "SERVER",
      keys: "KUNCI AI",
      ...
      online: "ONLINE",
      offline: "OFFLINE",
      delConfirm: "Hapus member ini selamanya?",
      guide: "Atur akses member dan isi ulang kredit.",
      guideTitle: "ADMIN PANEL",
      credits: "Kredit"
    },
    en: {
      title: "Manage Users",
      subtitle: "Studio User Registry",
      nodes: "USERS",
      health: "SERVER",
      keys: "AI KEYS",
      ...
      online: "ONLINE",
      offline: "OFFLINE",
      delConfirm: "Delete this user forever?",
      guide: "Manage user access and credits.",
      guideTitle: "ADMIN PANEL",
      credits: "Credits"
    }
  }[lang];
  ...
  const updateMemberStatus = async (member: Member, status: 'active' | 'inactive') => {
    let validUntil = null;
    if (status === 'active') {
      const nameStr = member.full_name || '';
      let days = 30;
      if (nameStr.includes('3B')) days = 90;
      else if (nameStr.includes('1T')) days = 365;
      
      validUntil = new Date(Date.now() + days * 24 * 60 * 60 * 1000).toISOString();
    }
    
    await supabase.from('members').update({ status, valid_until: validUntil }).eq('id', member.id);
    fetchMembers();
  };
        ...
        {activeTab === 'members' && (
          <motion.div key="mem" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {members.filter(m => m.email.toLowerCase().includes(searchTerm.toLowerCase()) || (m.full_name && m.full_name.toLowerCase().includes(searchTerm.toLowerCase()))).map(m => {
                ...
                return (
                  <div key={m.id} className="glass-panel p-6 rounded-[2rem] bg-[#0d1117] border border-white/5 shadow-2xl group transition-all hover:border-cyan-500/20">
                    ...
                    <div className="grid grid-cols-4 gap-1 mb-4">
                       ...
                       <button onClick={() => quickTopup(m.email, 1000)} className="py-2 bg-white/5 rounded-lg text-[8px] font-black text-cyan-500 hover:bg-cyan-500 hover:text-black">+1rb</button>
                       <button onClick={() => quickTopup(m.email, 5000)} className="py-2 bg-white/5 rounded-lg text-[8px] font-black text-cyan-500 hover:bg-cyan-500 hover:text-black">+5rb</button>
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-white/5">
                      ...
                      <button onClick={() => updateMemberStatus(m, m.status === 'active' ? 'inactive' : 'active')} className={`px-6 py-2 rounded-xl text-[9px] font-black uppercase transition-all shadow-lg ${m.status === 'active' ? 'text-red-500 bg-red-500/10' : 'text-black bg-cyan-500'}`}>
          ...
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="glass-panel w-full max-w-sm p-8 rounded-[2.5rem] bg-[#0d1117] border-white/10 shadow-2xl">
              ...
              <form onSubmit={handleUpdateIdentity} className="space-y-5">
                ...
                <div className="space-y-1">
                   <label className="text-[8px] font-black text-slate-600 uppercase tracking-widest px-1">Ganti Sandi (Opsional)</label>
                   <input type="password" value={editPassword} onChange={e => setEditPassword(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-2xl py-4 px-5 text-sm text-white" placeholder="Sandi baru..." />
                </div>
