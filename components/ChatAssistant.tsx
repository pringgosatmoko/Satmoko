...
import { rotateApiKey, getActiveApiKey } from '../lib/api';
...
export const ChatAssistant: React.FC<ChatAssistantProps> = ({ onBack, lang }) => {
  ...
  const handleSend = async (manualText?: string, retryCount = 0) => {
    ...
    if (!apiKey) {
      setMessages(prev => [...prev, { role: 'assistant', text: "Waduh Bro, API Key Master belum terdeteksi. Coba cek Console Master dulu ya!" }]);
      setIsTyping(false);
      return;
    }
    ...
    try {
      ...
      if ((msg.includes('429') || msg.includes('quota') || msg.includes('API key')) && retryCount < 3) {
        rotateApiKey();
