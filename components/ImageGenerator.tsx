...
export const ImageGenerator: React.FC<ImageGeneratorProps> = ({ onBack, userEmail, credits, refreshCredits }) => {
  ...
  const generateImage = async (retryCount = 0) => {
    ...
    try {
      ...
      const modelName = modelType === 'Nano Banana Pro' ? 'gemini-2.5-pro' : 'gemini-2.5-flash-image-preview';
      ...
      const response = await ai.models.generateContent({
        ...
        config: { 
          imageConfig: { 
            ...
            imageSize: modelName === 'gemini-2.5-flash-image-preview' ? '1K' : undefined
          } 
        }
      });
