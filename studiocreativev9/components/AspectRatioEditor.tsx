...
export const AspectRatioEditor: React.FC<AspectRatioEditorProps> = ({ onBack, lang, userEmail, credits, refreshCredits }) => {
  ...
  const processReframe = async (retryCount = 0) => {
    ...
    try {
      ...
      // PROMPT YANG DIPERBAIKI: Fokus pada ekstensi latar belakang murni, melarang refleksi/mirroring.
      const prompt = `Extend the background of this image to fit a ${aspectRatio} aspect ratio. 
      INSTRUCTIONS: 
      1. Use "Generative Fill" to outpaint new areas. 
      2. CONTINUE the existing background patterns, textures, and lighting naturally. 
      3. ABSOLUTELY NO MIRRORING, NO REFLECTIONS, and NO DUPLICATIONS of the main subject at the bottom or sides. 
      4. If it's a solid background, extend the gradient. If it's a floor, continue the perspective of the floor. 
      5. Keep the original product and person untouched. Realistic and professional high-end photography result.`;
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          temperature: 0.75, // Mengurangi suhu sedikit untuk menjaga konsistensi
        },
      });
