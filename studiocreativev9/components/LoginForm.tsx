  ...
  const handleStartPayment = async () => {
    ...
    try {
      ...
      if (!response.ok || !data.token) {
        throw new Error(data.error || "Gagal mendapatkan akses pembayaran. Cek Server Key Master.");
      }
    ...
    <div className="w-full flex flex-col">
      <div className="glass-panel p-8 rounded-[2.5rem] bg-black/60 border border-white/5 shadow-2xl relative">
        ...
        <AnimatePresence mode="wait">
          {error && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 p-4 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] text-center font-black uppercase italic whitespace-pre-wrap">
      ...
      <button onClick={() => setMode(mode === 'login' ? 'register' : 'login')} className="w-full mt-10 mb-10 text-[9px] font-black uppercase tracking-[0.4em] text-slate-600 hover:text-cyan-400 transition-colors text-center">
